...
load_dotenv()
# ==========================================
# 1. INIZIALIZZAZIONE API CLIENT (DALL'ENV)
# ==========================================
...
BASELINES = {
    "Baseline-8B": {
        ...
        "output_file": "baseline_8b_risultati.json",
    },
}
...
for nome_baseline, info in BASELINES.items():
    print(f"\n🚀 Esecuzione {nome_baseline} su Groq ({info['model_name']})...")
    risultati_esempi = []
    ...
    if not risultati_esempi:
        print(
            f"❌ Nessun dato estratto per {nome_baseline}. Verifica l'ID modello su Groq."
        )
        continue
